...
def show_busan_map(request):
    def get():
        template = loader.get_template('apitest/main.html')
...
def indexP(request):
    ...
    for item in code_list:
        params = '?' + urlencode({ quote_plus('ServiceKey') : 'z6wOy%2Bx%2BzNx%2F7dhcKcU6E02LweiSbcLwhYO0SJEyAtiBTOROAvH2czsCuJgui8o7hOORZ4O13Mw43aKzPml3Kg%3D%3D'\
                            , quote_plus('pageNo') : '1', quote_plus('numOfRows') : '160', quote_plus('dataType') : 'JSON', quote_plus('base_date') : dates, quote_plus('base_time') : times, quote_plus('nx') : str(item[0]), quote_plus('ny') : str(item[1]) })
        # URL parsing
